...
test('should always return previous state after each update', async (t) => {
  const { result, rerender } = renderHook((props) => usePrevious(props.state), { initialProps: { state: 0 } });
...
test('accepts an initial value', async (t) => {
  const { result, rerender } = renderHook((props) => usePrevious(props.state, 10), { initialProps: { state: 0 } });
